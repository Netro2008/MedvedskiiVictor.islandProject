...
public class Sheep extends Animal {
    ...
    public final static double weightKilograms = 70;
    private final static int maxAnimalsOfThisTypeInOneCell = 140;
    private final static int numberOfMaxCellsAnimalCanMove = 3;
    private final static double kilogramsTheyNeedToEatToBeFull = 15;
    public static double ateKilogramsForNow;

    public Sheep(double weightKilograms, int maxAnimalsOfThisTypeInOneCell, int numberOfMaxCellsAnimalCanMove, double kilogramsTheyNeedToEatToBeFull, int x, int y) {
        super(weightKilograms, maxAnimalsOfThisTypeInOneCell, numberOfMaxCellsAnimalCanMove, kilogramsTheyNeedToEatToBeFull, x, y);
    }

    public static synchronized void createSheep() {
        Random random = new Random();
        int x = random.nextInt(6), y = random.nextInt(6);
        Sheep.x = x;
        Sheep.y = y;
        new Sheep(weightKilograms, maxAnimalsOfThisTypeInOneCell, numberOfMaxCellsAnimalCanMove, kilogramsTheyNeedToEatToBeFull, x, y);
        Statistics.animalCounter++;
    }

    private static final int SHEEP_EATING_PROBABILITY = 100;
    ...
    protected void eat(Animal animal) {
        if (ateKilogramsForNow < kilogramsTheyNeedToEatToBeFull) {
            ...
            && !(animal instanceof Goat) && !(animal instanceof Duck) && !(animal instanceof Deer) && !(animal instanceof Caterpillar)
                ...
                boolean eaten = Randomizer.getProbability(SHEEP_EATING_PROBABILITY);
                if (eaten) {
