...
public class Deer extends Animal {
    ...
    public final static double weightKilograms = 300;
    ...
    public Deer(double weightKilograms, int maxAnimalsOfThisTypeInOneCell, int numberOfMaxCellsAnimalCanMove, double kilogramsTheyNeedToEatToBeFull, int x, int y) {
        super(weightKilograms, maxAnimalsOfThisTypeInOneCell, numberOfMaxCellsAnimalCanMove, kilogramsTheyNeedToEatToBeFull, x, y);
    }

    public static synchronized void createDeer() {
        Random random = new Random();
        int x = random.nextInt(6), y = random.nextInt(6);
        Deer.x = x;
        Deer.y = y;
        new Deer(weightKilograms, maxAnimalsOfThisTypeInOneCell, numberOfMaxCellsAnimalCanMove, kilogramsTheyNeedToEatToBeFull, x, y);
        Statistics.animalCounter++;
    }

    private static final int DEER_EATING_PROBABILITY = 100;
    ...
    protected void eat(Animal animal) {
        if (ateKilogramsForNow < kilogramsTheyNeedToEatToBeFull) {
            if (!(animal instanceof Wolf) && !(animal instanceof SnakeBoa) && !(animal instanceof Fox) && !(animal instanceof Eagle) && !(animal instanceof Bear)
                ...
                boolean eaten = Randomizer.getProbability(DEER_EATING_PROBABILITY);
                if (eaten) {
